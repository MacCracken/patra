...
| **Symlink / TOCTOU** | `O_NOFOLLOW` on `_pt_file_open`, `_pt_file_create`, `jsonl_open`, and (since v1.14.0) the WAL's `wal_start` / `wal_recover`. A symlinked target fails with `ELOOP` on Linux and macOS. ⚠ **Windows does not enforce `O_NOFOLLOW`** (cyrius defines it for portability only): a planted symlink redirects the open, and `_pt_file_create`'s `O_EXCL` create over a *dangling* link creates the link's target. |
...
| Multiple processes on the same host, cooperating via `flock` | Supported (advisory locking model), with **one open gap**: a process that dies mid-transaction while another holds the database open is recovered only at the next `patra_open`. Until then the survivor reads the dead transaction's uncommitted rows, a `BEGIN` of its own makes them permanent, and an autocommit write of its own is lost when the WAL is later replayed. See `docs/development/issues/2026-09-23-wal-recovery-runs-only-at-open.md`. Pre-existing; not fixed in 1.15.0. |
...
| Targets other than Linux x86_64 | Since v1.15.0 every kernel touch goes through a cyrius stdlib wrapper and every `open(2)` flag is an `O_*` symbol (CI-gated), so syscall numbers and flag values are per-target rather than x86_64 Linux. **Linux aarch64**: the full suite passes under `qemu-aarch64`, checked by hand, not in CI. **macOS, agnos, Windows**: cross-built, never run. **Windows is suitable only for single-process, non-durable use**: no flush is wired, `flock` is absent (so WAL recovery never runs on open, and a transaction interrupted by a crash stays partly applied), and `O_NOFOLLOW` is not enforced. **On agnos, contended locks do not hold across processes**: `flock`#59 never waits and patra ignores the failure, so neither writers nor readers are serialized between processes (filed with agnos: `agnos/docs/development/issues/2026-09-23-flock-never-waits-and-no-caller-spins.md`). Details: `docs/development/roadmap.md` *Platforms*. |
