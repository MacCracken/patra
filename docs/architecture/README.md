...
| 003 | [page-cache-coherence.md](003-page-cache-coherence.md) | `pcache.cyr`, `page.cyr`, `lib.cyr` | P2: the opt-in page cache's coherence — invalidate-on-write (Variant I), the `HDR_COMMITGEN` gen gate, the publish-inside-`LOCK_EX` invariant, and the pre-existing BYTES/TEXT lazy-read TOCTOU caveat. |
