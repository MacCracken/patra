...
> **Last refresh**: 2026-06-25 (v1.12.6 — `patra_insert_row_or_ignore` (sit BYTES `OR IGNORE`) + INT-index `OR IGNORE` tombstone fix: CHANGELOG [1.12.6] (Added + Fixed); state.md Current block + 1.12.6 release row + counts (870 tests / 7 fuzz / 40 benches) + source layout + CI-gate line; README API + `[deps.patra]` tag→1.12.6; roadmap (no open consumer requests); BENCHMARKS.md dedup rows + currency note; sit request archived → `requests/archive/`. Prior: v1.12.5 — cyrius 6.2.44 pin + agnos port finished, agnos + `cyrius distlib` issues archived.) | **Refresh cadence**: when docs are touched, update the affected row.
...
| 📦 **Archive / frozen by design** | ~6 | `docs/adr/0001-...` (dated ADR; workaround still in place, re-verified under 6.2.19); `docs/audit/2026-04-21/security-review.md` (dated audit, frozen); **four** resolved-upstream issues under `issues/archive/` (cyrfmt buffer truncation; cyrius distlib blank-lines; no-portable-mutex; agnos cross-target ABI). |
...
|---|---|---|---|
| `README.md` | 2026-06-25 | ✅ Fresh | Correct init→open→exec→query→close snippet; "What It Does" lists the shipped features; module map + consumers current. `[deps.patra]` example tag tracks the release (now **1.12.6**); v1.12.6 documented `patra_insert_row_or_ignore` (BYTES `OR IGNORE`). |
| `CHANGELOG.md` | 2026-06-25 | ✅ Fresh | Source of truth for shipped work. Current through **1.12.6** (`patra_insert_row_or_ignore` + INT-index `OR IGNORE` tombstone fix). |
...
| File | Last touched | Status | Notes |
|---|---|---|---|
| `state.md` | 2026-06-25 | ✅ Fresh | Bumped at **v1.12.6** — version 1.12.6, pin 6.2.44, **870 tests / 7 fuzz / 40 benchmarks**, source line-counts (lib.cyr 2222), binary 281,728 bytes, Current block + the 1.12.2–1.12.6 release rows, footgun/resolved sync, CI-gate line. Refresh every release. |
...
| `BENCHMARKS.md` | 2026-06-25 | ✅ Fresh (baseline + dated additions) | Legacy rows remain the v1.9.5 / cyrius 6.0.1 baseline; **v1.12.6** added the BYTES-path dedup rows (`dedup_insert_row_or_ignore_500` ~10 µs vs workaround ~273 µs, ~26×) and bumped the currency note to v1.12.6 / 6.2.44 / 40 benches. Legacy re-baseline still deferred. |
...
*Last refresh: 2026-06-25 (v1.12.6 — `patra_insert_row_or_ignore` + INT-index `OR IGNORE` tombstone fix; CHANGELOG / state.md / README / roadmap / BENCHMARKS / requests synced; the header row above carries the per-file detail). Refresh in place when docs are touched.*
