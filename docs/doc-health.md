...
> **Last refresh**: 2026-06-25 (v1.12.5 — cyrius pin 6.2.28→6.2.44 + agnos port finished: WAL `sys_unlink`→`xunlink`; CHANGELOG [1.12.5]; state.md Current block + 1.12.2–1.12.5 release rows + footgun/resolved sync; roadmap (Current/Upstream); the **agnos cross-target ABI** + **`cyrius distlib` blank-lines** issues confirmed dead & archived — `issues/` now holds only `archive/`). Prior: 2026-06-18 (v1.12.0 — P2 concurrent readers: README concurrency + opt-in-cache + BYTES-caveat rewrites; two new ADRs 0002/0003 + three architecture notes 001–003). | **Refresh cadence**: when docs are touched, update the affected row.
