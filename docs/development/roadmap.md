...
### ~~1.13.8 — bind the WAL to its database~~ ✅ SHIPPED 2026-08-18

Both items closed. The WAL now carries its owning database's `HDR_DBID`
(**format v3 → v4**); v2/v3 replay best-effort with a page-existence check
rather than being refused, so a genuinely-crashed older database still gets its
undo. WHERE type mismatches return `PATRA_ERR_TYPE`, validated once per
statement across all three exec paths, scoped to genuine INT/STR mismatches.

**This closes the 1.13.x repair arc** — 1.13.2 memory safety, 1.13.3
transactions, 1.13.4 durability, 1.13.5 malformed-file, 1.13.6 wrong answers,
1.13.7 gates, 1.13.8 WAL binding. 893 → **1059 tests**, 7 → **8 fuzz harnesses**,
and four CI gates that did not exist before.
